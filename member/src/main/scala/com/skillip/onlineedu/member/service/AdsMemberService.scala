...
import com.skillip.onlineedu.member.bean.QueryResult
import com.skillip.onlineedu.member.dao.DwsMemberDao
...
object AdsMemberService {
  def queryDetailApi(spark: SparkSession, dt: String) = {
    ...
    val result = DwsMemberDao.queryIdlMemberData(spark).as[QueryResult]
      .where(s"dt='$dt'")
    result.cache() // ??????
